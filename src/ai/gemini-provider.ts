 ...
 */
const geminiLlmApiFetcher = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    try {
        ...
        if (error instanceof Error && error.name === 'AbortError') {
            throw new Error(`Gemini API request timed out after ${GEMINI_API_TIMEOUT / 1000} seconds`);
        }
        throw error;
    } finally {
